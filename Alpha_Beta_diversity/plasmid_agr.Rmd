---
title: "Plasmid antibiotic resistance gene occurrence diversity analysis"
...
```{r, warning=FALSE}
# Load counts
agr_genes_occurence <- read.csv("plasmid_agr_occurence.csv", header = TRUE,
                       skipNul = TRUE, sep = "\t", as.is = TRUE)
#agr_genes_occurence
#str(agr_genes_occurence)
#row.names(agr_genes_occurence)
#class(agr_genes_occurence)
colnames(agr_genes_occurence)
#head(colnames(agr_genes_occurence))
#dim(agr_genes_occurence)
#agr_genes_occurence$phage

## Put this in `matrix` class for `phyloseq`'s `otu table`, and create it:
counts <- as.matrix(sapply(agr_genes_occurence, as.numeric))
#head(counts)
#head(colnames(counts))
rownames(counts) <- agr_genes_occurence$ARG
#head(counts)
#head(colnames(counts))

# delete/remove the newly created "phage" column:
counts <- counts[ , -grep("ARG", colnames(counts))]
#head(counts)
#head(colnames(counts))
#dim(counts)
...
#otu.table
#get otu.table ARGs name from the index
rownames(otu.table)
...
```{r, warning=FALSE}
taxmat = matrix(sample(letters, 87, replace = TRUE),
...
```{r, warning=FALSE}
# Function to calculate mean and SD, ignoring NaN values and handling Inf
calc_mean_sd <- function(x) {
  x_clean <- x[!is.nan(x) & is.finite(x)]
  ...
  if (length(x_clean) > 0) {
    ...
    c(mean = NA, sd = NA, n = 0, n_total = length(x),
      n_inf = sum(is.infinite(x)), n_nan = sum(is.nan(x)))
  }
}

# Calculate statistics for each index by group
  ...
  summarise(
    across(c(Observed, Chao1, ACE, Shannon, InvSimpson),
           list(Mean = ~calc_mean_sd(.)[1],
                SD = ~calc_mean_sd(.)[2],
                N = ~calc_mean_sd(.)[3],
                N_Total = ~calc_mean_sd(.)[4],
                N_Inf = ~calc_mean_sd(.)[5],
                N_NaN = ~calc_mean_sd(.)[6]))
  )
...
write.csv(richness_stats, "group_richness_statistics_detailed.csv", row.names = FALSE)
```
...
```{r, warning=FALSE}
richness.table$InvSimpson[which(is.infinite(richness.table$InvSimpson))] <- NA
